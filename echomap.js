window.addEventListener("DOMContentLoaded", () => {
  ...
  function bindBuildingFootprintEvents(control) {
    ...
    polygon.on("mouseover", () => highlightBuildingFootprint(config.id));
    polygon.on("mouseout", () => syncBuildingFootprintStyle(config.id));
  ...
  function createPinIcon(color, icon) {
    ...
    return L.divIcon({
      ...
      html: `<div style="width:34px;height:34px;background:${markerColor};border:2px solid #fff;border-radius:50% 50% 50% 0;transform:rotate(-45deg);box-shadow:0 7px 16px rgba(44,31,20,.28);position:relative;"><div style="transform:rotate(45deg);font-size:14px;position:absolute;inset:0;display:grid;place-items:center;line-height:1;">${escapeHtml(markerIcon)}</div></div>`,
