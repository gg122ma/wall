...
function getRoute() {
  ...
  if (parts[0] === "wall" && parts[3]) {
    return { page: "wall", orgId: toId(parts[1]), batchId: toId(parts[2]), majorId: toId(parts[3]) };
  }
  return { page: "home" };
}
...
function render() {
  ...
  if (route.page === "home") renderHome(app);
    ...
    const batch = batches.find(item => item.id === route.batchId && item.orgId === route.orgId);
    ...
    if (!org || !batch || !major) {
      app.innerHTML = `
        <section class="container error-page page-reveal">
          ...
          <p>The organization, batch, and stream in this link do not belong together.</p>
          <button class="btn btn-primary" onclick="navigate('#/')">${I18n.t("org.back")}</button>
        </section>`;
    } else {
      renderWall(app, route.orgId, route.batchId, route.majorId);
    }
  }
      ...
      <section class="hero container">
        <div class="hero-copy page-reveal">
          ...
          <div class="hero-trust-row">
            ...
            <span><b>Batch-based</b> browsing</span>
...
function renderOrgDetails(container, orgId) {
  ...
  const orgBatches = batches.filter(item => item.orgId === orgId);
  const orgMajors = majors.filter(item => item.orgId === orgId);
  if (!orgBatches.some(item => item.id === selectedBatch)) selectedBatch = orgBatches[0]?.id || null;
  if (!orgMajors.some(item => item.id === selectedMajor)) selectedMajor = orgMajors[0]?.id || null;

  const batchItems = orgBatches.map((batch, index) => `
    <button class="selection-item ${selectedBatch === batch.id ? "selected" : ""}" style="--item-delay:${index * 55}ms" onclick="selectBatchItem(${batch.id})" aria-pressed="${selectedBatch === batch.id}">
      <span class="selection-icon">🎓</span><span><strong>${escapeHtml(batch.label)}</strong><small>${batch.year} cohort</small></span><span class="selection-check">✓</span>
    </button>`).join("");
...
function enterWallCanvas(orgId) {
  const batchIsValid = batches.some(item => item.id === selectedBatch && item.orgId === orgId);
  const majorIsValid = majors.some(item => item.id === selectedMajor && item.orgId === orgId);
  if (!batchIsValid || !majorIsValid) {
    showToast("Please select a valid batch and stream.");
    return;
  }
  navigate(`#/wall/${orgId}/${selectedBatch}/${selectedMajor}`);
}
